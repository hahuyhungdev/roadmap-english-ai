...
export default function LessonAssistant({
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full shadow-lg shadow-indigo-200 flex items-center justify-center z-50 transition-all active:scale-95"
  ...
  return (
    <section
      className="fixed bottom-6 right-6 w-[23rem] bg-white border border-gray-200 rounded-2xl shadow-xl shadow-gray-200/60 flex flex-col z-50 overflow-hidden"
    ...
    >
            ...
            <button
              ...
              className={`flex-1 text-xs px-2 py-1.5 rounded-lg font-medium transition-all ${
                mode === m
                  ...
                  : "text-gray-500 bg-white border border-gray-200 hover:border-indigo-200 hover:text-indigo-600"
              }`}
            >
              {MODE_LABELS[m]}
            </button>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
            ...
            <div className="flex flex-wrap gap-1 justify-center mt-1">
              {["Explain this", "Give examples", "Quiz me"].map((hint) => (
                <button
                  ...
                  className="text-[10px] px-2 py-1 rounded-full bg-white border border-gray-200 text-gray-500 hover:border-indigo-200 hover:text-indigo-600 transition-colors"
                >
                  {hint}
                </button>
              ))}
        ...
        {loading && (
          <div className="inline-flex items-center gap-2 text-xs text-gray-400 bg-gray-100 rounded-2xl px-3 py-2.5 rounded-bl-md">
      ...
      <form
        onSubmit={onSubmit}
        className="shrink-0 px-3 py-3 border-t border-gray-100 bg-white"
      >
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-800 outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 resize-none transition-all placeholder:text-gray-400"
            style={{ maxHeight: "7rem" }}
          />
          <button
            type="submit"
            disabled={loading || !input.trim()}
            className="w-9 h-9 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white rounded-xl flex items-center justify-center transition-colors shrink-0 active:scale-95"
            aria-label="Send"
          >
            <Send size={13} />
          </button>
