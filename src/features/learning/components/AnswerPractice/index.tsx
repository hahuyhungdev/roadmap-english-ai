    ...
    return (
      <div className="fixed bottom-5 left-5 z-50">
        <button
          ...
          className="flex items-center gap-2 px-3.5 py-2 bg-zinc-800 hover:bg-zinc-700 text-white text-base font-semibold rounded-xl shadow-lg transition-all active:scale-[0.96]"
        >
          <BookOpen size={13} />
          Q&amp;A
        </button>
      ...
      <div className="relative z-10 w-full max-w-5xl h-[78vh] flex flex-col rounded-2xl overflow-hidden shadow-2xl bg-zinc-900 border border-zinc-700">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-700 shrink-0">
          <div className="flex items-center gap-2 text-base font-semibold text-zinc-100">
            ...
            {lessonTitle && (
              <span className="text-base font-normal text-zinc-400 truncate max-w-[200px]">
                — {lessonTitle}
              </span>
            )}
          </div>
          <div className="flex items-center gap-1.5">
            <button
              ...
              disabled={loading}
              title="Regenerate"
              className="flex items-center gap-1.5 text-base px-2.5 py-1.5 rounded-lg text-zinc-400 hover:text-zinc-100 hover:bg-zinc-700 transition-colors disabled:opacity-40"
            >
        ...
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 text-zinc-500">
            ...
            <p className="text-base text-zinc-400">Generating coaching cards…</p>
            <p className="text-base text-zinc-600">15–30 seconds</p>
          </div>
        ) : error ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 px-8">
            ...
            <button
              ...
              className="text-base px-4 py-2 bg-zinc-700 hover:bg-zinc-600 text-zinc-200 rounded-xl transition-colors"
            >
              Try again
            </button>
          </div>
            ...
            <main className="flex-1 overflow-y-auto">
              {!selected ? (
                <div className="flex flex-col items-center justify-center h-full gap-2 text-zinc-600">
                  <List size={24} />
                  <p className="text-base">Pick a question</p>
                </div>
              ) : (
                <CoachingCard
                  key={selected.id}
...
function GroupList({
  ...
  return (
    <div>
      ...
      >
        <span className="text-base font-bold text-emerald-500 uppercase tracking-widest">
          {group.label}
        </span>
            ...
            return (
              <li key={q.id}>
                <button
                  onClick={() => onSelect(q)}
                  className={clsx(
                    "w-full text-left px-2.5 py-1.5 rounded-lg text-base flex items-start gap-2 transition-colors",
                    active
                      ? "bg-emerald-600/20 text-emerald-300"
                      : "text-zinc-400 hover:bg-zinc-700/60 hover:text-zinc-200",
                  )}
                >
                  <span className={clsx("shrink-0 font-mono mt-0.5 text-base", active ? "text-emerald-500" : "text-zinc-600")}>
                    {i + 1}.
                  </span>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-7 pt-6 pb-5 border-b border-zinc-700 shrink-0">
        <p className="text-base font-bold text-emerald-500 uppercase tracking-widest mb-2">
          {question.group}
        </p>
      ...
      <div className="flex-1 overflow-y-auto px-7 py-6 space-y-6">
        {question.framework && (
          <Section icon={<Lightbulb size={14} className="text-amber-400" />} title="How to structure" accent="amber">
            <p className="text-base text-zinc-300 leading-relaxed">{question.framework}</p>
          </Section>
        )}
          ...
          <Section icon={<List size={14} className="text-indigo-400" />} title="Ideas to cover" accent="indigo">
            <ul className="space-y-2">
              {question.keyPoints.map((pt, i) => (
                <li key={i} className="flex items-start gap-2.5 text-base text-zinc-300">
      ...
      <div className="shrink-0 px-7 py-4 border-t border-zinc-700 flex items-center justify-between">
        <button
          ...
          disabled={!prev}
          className="flex items-center gap-1.5 text-base text-zinc-500 hover:text-zinc-200 disabled:opacity-25 transition-colors"
        >
          <ChevronRight size={14} className="rotate-180" /> Prev
        </button>
        <span className="text-base text-zinc-600">{idx + 1} / {allQuestions.length}</span>
        <button
  ...
  return (
    <div className={clsx("pl-3.5 border-l-2", border)}>
      <div className="flex items-center gap-2 mb-2">
        {icon}
        <span className="text-base font-semibold text-zinc-500 uppercase tracking-wide">{title}</span>
      </div>
      {children}
    </div>
  );
  ...
  return (
    <div className="rounded-lg border border-zinc-700 overflow-hidden">
      <button
        onClick={() => setOpen((v) => !v)}
        className={clsx(
          "w-full flex items-center justify-between px-3 py-2 text-left text-base font-medium transition-colors",
      ...
      {open && (
        <div className="px-4 py-3 bg-zinc-800/50">
          <p className="text-base text-zinc-300 leading-relaxed">{text}</p>
        </div>
      )}
    </div>
  );
